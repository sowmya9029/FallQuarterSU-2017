...
CREATE TABLE `books` (
  ...
  KEY `user_id_idx` (`user_seller_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;
...
--
-- Temporary table structure for view `line_item_detials`
--

DROP TABLE IF EXISTS `line_item_detials`;
/*!50001 DROP VIEW IF EXISTS `line_item_detials`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE TABLE `line_item_detials` (
  `order_id` tinyint NOT NULL,
  `sum(line_item_quantity)` tinyint NOT NULL,
  `sum(line_item_price)` tinyint NOT NULL,
...
--
-- Final view structure for view `line_item_detials`
--

/*!50001 DROP TABLE IF EXISTS `line_item_detials`*/;
/*!50001 DROP VIEW IF EXISTS `line_item_detials`*/;
...
/*!50001 VIEW `line_item_detials` AS select `line_item`.`order_id` AS `order_id`,sum(`line_item`.`line_item_quantity`) AS `sum(line_item_quantity)`,sum(`line_item`.`line_item_price`) AS `sum(line_item_price)`,count(`line_item`.`order_id`) AS `number_of_items` from `line_item` group by `line_item`.`order_id` */;
...
-- Dump completed on 2017-11-29 16:54:52
